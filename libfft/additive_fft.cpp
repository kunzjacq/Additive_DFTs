    ...
    )  const
{
  ...
  for(step = first_step; step < c_b_t<word>::n; step++)
  {
    ...
    if(debug)
    {
      ...
      cout << " reducing by P_" << i << ", of degree " << reductor_d << endl;
    ...
    for(j = 0; j < num_blocks; j++)
    {
      ...
      {
        cout << "j = " << j << endl;
        cout << "r+q*(P_i+cst):" << endl;
        print_series<word>(source, o, min(ho, 8uL));
      }
      euclidean_division(
            m_c_b, source, reductee_d, packed_degrees + c_b_t<word>::n * i, cst_term, num_coefficients[i],
            reductor_d, buf, dividend);
      if(debug && j <= 1)
      {
        cout << "r:" << endl;
        print_series<word>(buf, ho, min(ho, 8uL));
        if(compute_dividend)
        {
          cout << "q:" << endl;
          print_series<word>(buf + ho, ho, min(ho, 8uL));
    ...
    )  const
{
  ...
  for(step = first_step; step != last_step - 1 ; step--)
  {
    ...
    for(j = 0; j < num_blocks; j++)
    {
      ...
      // now
      // buf = dividend (degree < ho)
      // poly_ij =  remainder + dividend * cst_term (degree < ho)
      for(uint64_t ii = 0; ii < num_coefficients[i]; ii++)
      {
        uint64_t degree = packed_degrees[c_b_t<word>::n * i + ii];
        for(k = 0; k < ho; k++)
        {
...
/*
 * slower than rev_ref, because by not using a buffer, we have to resort to a less favorable
 * loop order.
 * see additive_fft_rev_fast_in_place for a version with better memory locality thanks
 * to deinterlacing.
 */
template<class word>
    ...
    )  const
{
  ...
  for(step = first_step; step < c_b_t<word>::n; step++)
  {
    ...
    const bool compute_dividend = ho < blk_size;
    //if compute_dividend=true, num_blocks is equal to 1
    for(j = 0; j < num_blocks; j++)
    {
    ...
    uint64_t ii, ik;
    const uint64_t hos  = num_blocks * ho;
    const uint64_t kmax = compute_dividend? num_blocks * o : ho;
    ...
    for (ii = reductee_d; ii > reductor_d - 1; ii--)
    {
      ...
      if (compute_dividend)
      {
        word* buf_off2 = buf + ((ii - reductor_d) * num_blocks) + hos;
      ...
      {
        uint64_t idx = (packed_degrees[c_b_t<word>::n * i + ik] + ii_mod_reductor_degree) & (reductor_d-1);
        // idx = (packed_degrees[m * i + ik] + ii) % reductor_d
    ...
    if(step == first_step)
    {
      for (ii = 0; ii < reductor_d; ii++)
      {
        ...
        word coeff = p_poly[ii];
        for(j = 0; j < num_blocks; j++) buf_off4[j] ^= coeff;
      }
    }
    else
    {
