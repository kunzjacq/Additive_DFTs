 ...
 */
template <int s, int logstride, int logsize>
inline void mgtt_aux(
...
// should be <= 2**s, the input is assumed to be of size 2**logsize and only the 2**logsize first output values are computed
{
  ...
  else
  {
    ...
    if(logsize <= t)
    {
      mgtt_aux<s - 1, logstride, logsize>(mult_pow_table, poly, offsets_mult, first_taylor_done);
    }
    else
    {
      // here 2*t >= logsize > t
      ...
      //offset' = offset >> t;
      mgtt_aux<s - 1, logstride + t, logsize - t>(mult_pow_table, poly, offsets_mult + t, false);
      ...
      for(uint64_t i = 0; i < tau; i++)
      {
        ...
        mgtt_aux<s - 1, logstride, t>(mult_pow_table, poly, offsets_local, false);
...
inline void mgt_aux(
    ...
    bool first_taylor_done)
{
  if(logsize == 1) mgtt_aux<s, logstride, 1>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 2) mgtt_aux<s, logstride, 2>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 3) mgtt_aux<s, logstride, 3>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 4) mgtt_aux<s, logstride, 4>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 5) mgtt_aux<s, logstride, 5>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 6) mgtt_aux<s, logstride, 6>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 7) mgtt_aux<s, logstride, 7>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 8) mgtt_aux<s, logstride, 8>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 9) mgtt_aux<s, logstride, 9>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 10) mgtt_aux<s, logstride, 10>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 11) mgtt_aux<s, logstride, 11>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 12) mgtt_aux<s, logstride, 12>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 13) mgtt_aux<s, logstride, 13>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 14) mgtt_aux<s, logstride, 14>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 15) mgtt_aux<s, logstride, 15>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 16) mgtt_aux<s, logstride, 16>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 17) mgtt_aux<s, logstride, 17>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 18) mgtt_aux<s, logstride, 18>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 19) mgtt_aux<s, logstride, 19>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 20) mgtt_aux<s, logstride, 20>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 21) mgtt_aux<s, logstride, 21>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 22) mgtt_aux<s, logstride, 22>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 23) mgtt_aux<s, logstride, 23>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 24) mgtt_aux<s, logstride, 24>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 25) mgtt_aux<s, logstride, 25>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 26) mgtt_aux<s, logstride, 26>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 27) mgtt_aux<s, logstride, 27>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 28) mgtt_aux<s, logstride, 28>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 29) mgtt_aux<s, logstride, 29>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 30) mgtt_aux<s, logstride, 30>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 31) mgtt_aux<s, logstride, 31>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 32) mgtt_aux<s, logstride, 32>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 33) mgtt_aux<s, logstride, 33>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 34) mgtt_aux<s, logstride, 34>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 35) mgtt_aux<s, logstride, 35>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 36) mgtt_aux<s, logstride, 36>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 37) mgtt_aux<s, logstride, 37>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 38) mgtt_aux<s, logstride, 38>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
  else if(logsize == 39) mgtt_aux<s, logstride, 39>(mult_pow_table, poly, offsets_mult, logsize, first_taylor_done);
}
    ...
    )
{
  if constexpr(s > 0)
  {
    ...
    for(uint64_t i = 0; i < 1uLL << (logsize - logsizeprime); i++)
    {
      ...
      mgt_aux<s, 0>(mult_pow_table, p + (i << logsizeprime), offsets_mult, logsizeprime, true);
  ...
  else
  {
    ...
    offsets_mult[0] = 0;
    mgt_aux<0, 0>(mult_pow_table, p, offsets_mult, logsizeprime, false);
  }
}
    ...
    uint8_t* p1, uint8_t* p2, uint8_t* result, uint64_t d1, uint64_t d2)
{
  ...
  mgt_smalldegree<s>(m_mult_beta_pow_table, b1, logsizeprime, logsize);
  logsizeprime = logsize;
  while(1uLL << logsizeprime >= 2 * w2) logsizeprime--;
  mgt_smalldegree<s>(m_mult_beta_pow_table, b2, logsizeprime, logsize);
  if(logsize < 2)
  {
