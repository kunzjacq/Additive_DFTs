...
As a final remark, note that this algorithm requires to work with the basis $\Alpha=\{1, \alpha, \alpha^2,\ldots, \alpha^{n-1}\}$, which is distinct from the Cantor basis used for efficient DFT computation. Another incentive to use the basis $\Alpha$ is that, with a careful selection of the minimal polynomial of $\alpha$, it enables an efficient implementation of multiplication in $\mathbb{F}$.