# Additive FFT algorithms: Cantor, Mateer-Gao
...
This suffices to deduce the Von Zur Gathen - Gerhard FFT algorithm in $\mathbb{F}$:
...
If we deduce from $f = f_0 \mod \sigma_{2t}(x - \omega_{\tau j})$ the set of $f_0$ modulo the $R_k = \sigma_{t}(x) - \omega_{\tau j + k}$, $k < \tau$, it will be possible to finish the evaluation of $f_0$ on  $W_{2t} + \omega_{\eta j}$ by doing an FFT on each of the $R_k$, which will give the values of $f_0$ on the spaces $W_t + \omega_{\eta j + \tau k}$, $k< \tau$, which partition $W_{2t} + \omega_{\eta j}$.
...
In other words, the $\{f \mod R_k, k < \tau\}$ are evaluations of the polynomial $P(y)= \sum_{i<\tau} f_i(x) y^i$ on the points of the space $W_t + \omega_{\tau j}$. These evaluations can therefore be performed by an FFT. The fact that the coefficients of $P$  and the values to compute are themselves polynomials is not a problem: $\tau$ FFT are performed according to the degrees in $x$ and the final values are obtained by reassembling the FFT results.

The recursion step ends with an FFT of each of the $R_k$, which produces the desired values of $f$ (or $f_0$).
...
2) Computation of $\tau$ FFT of size $\tau$ on the columns of $M$ to obtain the values of the corresponding polynomials in $W_t + \omega_{\tau j}$; i.e. $\tau$ recursive calls with $s - 1$, coefficients, offset $j$.
Before the FFT, the column $k$ contains the polynomial in $y$
$$P_k = \sum_{i < \tau} f_{i,k} \ y^i$$
in other words, the coefficient at position $(i,k)$ is $f_{i,k}$.
After the FFT, the coefficient $(i,k)$ is
...
3) Computation of $\tau$ FFT of size $\tau$ on the rows of the matrix, i.e. for all $i < \tau$, the FFT of $f \mod R_i$ on $W_t + \omega_{\eta j + \tau i}$.
...
As the $i$-th FFT provides the values of $f$ on $W_t + \omega_{\eta j + \tau i}$, if the matrix is in row order in memory, the values are already in the right order for the final return of $\omega_{\eta j + k}$, $k = 0 \ldots \eta - 1$.

### Simplified code

In this prototyping, the `fft` function below performs $2^{\text{logstride}}$ FFT on as many interleaved polynomials, which simplifies the recursion and limits the number of function calls performed.

~~~C++
void fft(uint64_t* poly, int s, uint64_t j, unsigned int logstride)
{
    if(s==0)
    {
...
~~~
 ...
 The main implementation difficulty is to ensure simultaneously a good performance for the multiplication in $\mathbb{F}$ (function `multiply`) and for the evaluation of $j\mapsto \omega_j$ (function `omega`). `decompose_taylor` is not shown here but the algorithm is described in [1]. Interstingly, it can be implemented recursively or iteratively, and for a similar implementation effort, the recursive implementation exhibits better memory locality and is faster (in addition to being much simpler).
